...
def append_audit(
    ...
    *,
    ...
    def _json_or_empty(v: Any) -> str:
        if v is None:
            return ""
        if isinstance(v, str):
            return v
        return safe_json_string(v, "")

    before_json = _json_or_empty(before)
    after_json = _json_or_empty(after)
    ...
    if not cid:
        try:
            ...
            cid = str(getattr(g, "request_id", "") or "").strip()
        except Exception:
            cid = ""
    ...
    db.add(
        AuditLog(
            ...
            actorEmail=str(actor.email if actor else ""),
